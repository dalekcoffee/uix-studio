...
// Placeholder image — a baked 16:9 PNG (diagonal stripes + dashed border +
// centered image icon), generated by scripts/gen-placeholder.ps1 to match the
// editor's ImagePlaceholderOverlay. Baked WHITE/grayscale so the export can
// TINT it with the active theme's bodyTextColor (legible on every theme — see
// the useImagePlaceholder branch in compImage). 16:9 sprite into the fixed-16:9
// placeholder rect stretches 1:1 with no distortion.
const BUNDLED_IMAGE_PLACEHOLDER_URL =
  "@packdb:///305cfc1a4cf7f8a4ce0c0abdea3b757eb953b59a10125ca5bd1df06c11605e2a";
...
let _imagePlaceholderTexId = ""; let _imagePlaceholderSpriteId = ""; // placeholder image
...
function compImage(p: Record<string, unknown>): Record<string, unknown> {
  ...
  const wantsShape = !useCustom && !useIcon && !useLogo && !useImagePlaceholder;
  ...
  const tint = useImagePlaceholder
  ...
  const preserveAspect = bgFit
    ? bgFit === "fit"
    // The image-placeholder sprite is a baked 16:9 hatch+icon. Forcing
    // PreserveAspect keeps the centered "drop image here" icon proportional in
    // ANY rect (tall login art panels, wide banners) instead of stretching with
    // the slot — the authored preserveAspect:false is for the user's eventual
    // upload, not the placeholder glyph.
    : useImagePlaceholder
    ? true
...
function buildCanvasStack(
  ...
  const stackChildren: Record<string, unknown>[] = rows.map((row) => {
    if (row.length === 1) {
      ...
      if (it.rect && it.pinnedX && elemW < contentW - 2) {
        ...
        return {
          ...
          Components: { ID: rowCompId, Data: [
            ...
            { Type: typeIndex("LayoutElement"), Data: compLayoutElement({ minHeight: it.size.h, preferredHeight: it.size.h, priority: 1 }) },
          ] },
      ...
      setLayoutElementSize(it.ser, { preferredHeight: it.size.h });
    ...
    const rowKids = row.map((it) => {
      setLayoutElementSize(it.ser, { preferredWidth: it.size.w, preferredHeight: it.size.h });
      (it.ser as { ParentReference: string | null }).ParentReference = rowSlotId;
      return it.ser;
    });
