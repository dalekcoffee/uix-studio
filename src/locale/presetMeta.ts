...
const PRESET_META: Record<string, PresetMeta> = {
  ...
  "resopal-counter-buddy": {
    ...
    description: {
      ko: "구분선으로 두 섹션을 나눈 Palworld TCG 턴 트래커. 자원: −/+ 카운터 3개(라이프, 재료, 재료아이템)로, 각각 전용 자원 아이콘과 이름 있는 다이내믹 변수로 내보내지는 입력 가능한 정수 필드를 가집니다. 소울: 아래의 보유 / 소모 카운터 두 개를 그대로 보여주는 보라색 카드 10개, 소모 초기화 버튼, 남은 수 표시. 소울 하나는 레이어를 쌓은 게 아니라 색 하나를 가진 도형 하나라서, 전체 표시는 Tint 10개만 구동하면 됩니다 — 보라는 보유, 회색은 소모, 어두운 색은 미획득. ⓘ 대화 상자에는 1·2로 시작해 매 턴 +2씩 늘어나는 소울 규칙, 세 가지 색 범례, 연결 방법이 담겨 있습니다.",
      es: "Marcador de turnos para el TCG de Palworld en dos secciones separadas por una regla. RECURSOS: tres contadores −/+ (Vida, Material, Ingrediente), cada uno con su propio icono de recurso incluido y un campo de entero editable que se exporta como variable dinámica con nombre. ALMAS: una fila de diez fichas moradas que refleja el par de contadores DISPONIBLE / GASTADAS de debajo, más una píldora de reinicio y la cifra de restantes sobre el total. Una ficha es una sola forma con un solo color, no una pila de capas, así que toda la lectura son diez controles de Tint — morado disponible, gris gastada, oscuro sin obtener. El diálogo ⓘ trae las reglas de almas (1 y 2 al empezar, luego +2 por turno), una leyenda de los tres colores y las notas de conexión.",
      ja: "罫線で2つのセクションに分けた Palworld TCG のターントラッカー。リソース: −/+ カウンター3つ（ライフ、マテリアル、素材）。それぞれ専用のリソースアイコンと、名前付きのダイナミック変数として書き出される入力可能な整数フィールドを持ちます。ソウル: 下の 所持 / 消費 カウンター2つをそのまま映す紫のカード10枚と、消費リセットのピル、残り数の表示。ソウルはレイヤーの重ねではなく色1つを持つ図形1つなので、表示全体は Tint を10個駆動するだけです — 紫は所持、グレーは消費、暗い色は未獲得。ⓘ ダイアログには1・2で始まり毎ターン +2 増えるソウルのルール、3色の凡例、配線メモが入っています。",
    },
  },
  "basic-text": {
